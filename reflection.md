...
If I were explaining this to a friend, I'd put it this way: every button click is basically a full page refresh that re-executes all your code from scratch. Normal variables get wiped between those refreshes. The way you hold onto something across reruns is by storing it in `st.session_state`, which is like a small notepad that Streamlit keeps alive for the duration of the session. Whatever you write to it sticks around until you explicitly clear it.
